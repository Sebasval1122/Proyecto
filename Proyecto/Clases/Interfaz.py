...
class RestauranteGUI:
    ...
    def ingresar_queja(self):
        ...
        if queja and queja_mesa:
            try:
                self.app.ingresar_queja(queja, queja_mesa)
                messagebox.showinfo("Queja", f"Queja registrada: {queja} en la mesa {queja_mesa}")
            except Exception as e:
                messagebox.showerror("Error al registrar queja", str(e))
        else:
            messagebox.showerror("Error", "Debe ingresar una queja y una mesa válida.")

if __name__ == "__main__":
    root = tk.Tk()
    gui = RestauranteGUI(root)
    root.mainloop()